...
module Zendesk
  class API
    ...
    def open_ticket description, options = { }
      ...
      resource("/tickets", with_payload: true).post(payload, &method(:responder))
    end
    
    def get_ticket id
      ticket = resource("/tickets/#{id}").get(&method(:responder))
    ...
    def close_ticket id, options = { }
      ...
      resource("/tickets/#{id}", with_payload: true).put(payload, &method(:responder))
    end

    def delete_ticket id
      resource("/tickets/#{id}").delete(&method(:responder))
    end
    ...
    def create_user email, name, role_id, restriction_id
      ...
      resource("/users", with_payload: true).post(payload, &method(:responder))
    end
   
    def delete_user id
      resource("/users/#{id}").delete(&method(:responder))
    end
    
    def get_user id
      user = resource("/users/#{id}").get(&method(:responder))
      user["user"] unless user.nil?
    end
     
    def get_users
      resource("/users").get(&method(:responder))["users"]
    end
    ...
    def responder response, request, result, &block
      case response.code
